  ...
  if (data.price <= 0) {
    ...
    <div class="flex-shrink-0 w-72 bg-[#1b2838] text-white rounded-lg overflow-hidden shadow-md hover:scale-105 transition">
      ...
      <img src="${data.thumbnail}" class="w-full h-40 object-cover">
        ...
        <!-- Wishlist -->
        <button onclick="addToWishlist(${data.id})" class="absolute top-2 right-2 bg-black/60 p-2 rounded-full hover:bg-black">
...
function keranjang(id) {
  ...
  if (!p) {
    alert("berhasil ditambahakan ke cart");
    cart.push(selectedGame);
    console.log(cart);
    saveToLocal();
    console.log(cart);
  } else {
    alert("games sudah ada  di cart");
    return;
  }
}
